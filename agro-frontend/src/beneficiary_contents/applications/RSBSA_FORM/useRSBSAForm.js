...
export const useRSBSAForm = () => {
  ...
  const [formData, setFormData] = useState({
    beneficiaryDetails: {
        ...
        // CONTACT INFORMATION
        contact_number: null, // required
        emergency_contact_number: null,
        
        // PERSONAL INFORMATION
        birth_date: '',
        place_of_birth: null,
        sex: '', // required - start empty to force selection
        civil_status: null,
        name_of_spouse: null,
  ...
  const checkApplicationStatus = useCallback(async (userId) => {
    ...
    try {
      const response = await axiosInstance.get(`api/rsbsa/enrollments/user/${userId}/application_status`);
  ...
  const loadExistingEnrollment = useCallback(async (userId) => {
    ...
    try {
      ...
      const response = await axiosInstance.get(`api/rsbsa/enrollments/user/${userId}`);
      if (!isMountedRef.current) return;
      ...
      if (error.response?.status === 404) {
        console.log('No existing enrollment found for user (404)');
      } else {
        setBackendErrors(prev => ({ 
          ...prev, 
  ...
  // Updated addFarmParcel function to match backend structure
  const addFarmParcel = useCallback(() => {
    const newParcel = {
      id: Date.now(),
      farm_profile_id: null,
      sector_id: 1,                      // Set default sector_id (1 = Agriculture, adjust as needed)
      parcel_number: '',
      barangay: '',
      ...
      ownership_document_type: null,     // NEW: Added to match backend
      is_ancestral_domain: false,
      is_agrarian_reform_beneficiary: false,
      // REMOVED: farm_type (now in commodities)
      // REMOVED: is_organic_practitioner (now in commodities)
      remarks: '',
  ...
  // Updated validateForm to make farm data optional for farmworkers and agri-youth
...
// Updated canSubmit calculation to make farm data optional for farmworkers and agri-youth
